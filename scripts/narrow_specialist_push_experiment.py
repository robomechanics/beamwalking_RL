"""Push-train a gait specialist: warm start from its checkpoint, ramped base
perturbation on, everything else unchanged. See docs/perturbation.md and
docs/unified_gait_plan.md.
"""
...
from beam_walking.experiment.perturbation import (  # noqa: E402
    add_perturbation_arguments, perturbation_from_args)
from beam_walking.experiment.narrow_specialist import files_sha256, narrow_extension_record  # noqa: E402
from beam_walking.experiment.narrow_specialist_push import (  # noqa: E402
    PUSH_SCHEMA as SPECIALIST_SCHEMA,
    PUSH_TASK_FILES as UNIFIED_TASK_FILES,
    PUSH_TRAINING_ITERATIONS as UNIFIED_TRAINING_ITERATIONS,
    PUSH_TRAINING_KIND,
    PUSH_TRAINING_NUM_ENVS as UNIFIED_TRAINING_NUM_ENVS,
    PUSH_TRAINING_SEED as UNIFIED_TRAINING_SEED,
    PUSH_TRAINING_SOURCE_FILES as UNIFIED_TRAINING_SOURCE_FILES,
    push_lineage_id,
    verify_parent,
)
...
parser.add_argument("--initialize_from", type=Path,
                    help="Final checkpoint of the gait specialist to warm start from")
...
if perturbation_cfg is None:
    parser.error("The push fine-tune requires --perturbation")
if args.mode == "train" and args.initialize_from is None:
    parser.error("Training requires --initialize_from <specialist model_1799.pt>")
if args.mode == "train" and not args.development and (
    ...
    parser.error(
        "Primary push fine-tuning requires 3072 environments, 1800 updates, "
...
from beam_walking.experiment.narrow_specialist_task import (  # noqa: E402
    NarrowSpecialistEnv as UnifiedEnv,
    NarrowSpecialistEnvCfg as UnifiedEnvCfg,
    NarrowSpecialistPPORunnerCfg as UnifiedPPORunnerCfg,
)
...
def smoke(env, wrapped):
    ...
    required = {(gait, round(duty, 3)) for gait, duty in UNIFIED_STRATA
                if gait == GAITS.index(args.gait)}
    if sampled_strata - required:
        raise RuntimeError(f"Specialist sampled another gait: {sampled_strata - required}")
...
def main():
    ...
    cfg = UnifiedEnvCfg(specialist_gait=args.gait)
    ...
    env = perturbed_env_class(UnifiedEnv, perturbation_cfg)(cfg)
    wrapped = None
    try:
        ...
        lineage = None  # Bound to the parent checkpoint after it is loaded.
        provenance = {
            ...
            "gaits": [args.gait],
            "specialist_gait": args.gait,
            "gait_balance": "single_gait",
            "training_kind": PUSH_TRAINING_KIND,
            "external_pushes": True,
            "perturbation": perturbation_cfg.profile(),
            "protocol": "narrow_stance_single_gait_specialist_push_finetune",
            "narrow_beam_extension": narrow_extension_record(),
        ...
        if args.mode == "smoke":
            ...
            print("NARROW_SPECIALIST_PUSH_SMOKE_OK", json.dumps(result), flush=True)
            return

        runner = OnPolicyRunner(
            wrapped, agent.to_dict(), log_dir=str(args.output), device=env.device)
        parent = verify_parent(args.initialize_from, args.gait)
        state = parent.pop("parent_state")
        ...
        env.common_step_counter = parent["parent_common_step_counter"]
        wrapped.reset()
        provenance.update(parent)
        provenance["training_lineage_id"] = lineage
        (args.output / "provenance.json").write_text(json.dumps(provenance, indent=2))
        shutil.copy2(Path(parent["parent_checkpoint"]).parent / "provenance.json",
                     args.output / "parent_provenance.json")
        print("PUSH_PARENT_LOADED", json.dumps(
            {k: v for k, v in parent.items() if k != "parent_checkpoint"}), flush=True)
        ...
        print("NARROW_SPECIALIST_PUSH_TRAINING_COMPLETE", json.dumps(throughput), flush=True)
    finally:
